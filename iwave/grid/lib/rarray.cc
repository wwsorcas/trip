...
int ra_create(RARR *arr, IPNT gs, IPNT ge)
{
    ...
    //    for (int d=0; d<RARR_MAX_NDIM; d++) 
    //      fprintf(stderr,"  ra_create: d=%d s=%d e=%d\n",d,gs[d],ge[d]);
    return 0;
}
/*----------------------------------------------------------------------------*/
...
int ra_create_s(RARR *arr, IPNT gs, IPNT n)
{
    ...
    for ( d = 0; d < RARR_MAX_NDIM; ++d ) {
      ge[d] = gs[d] + n[d] - 1;
      //      fprintf(stderr,"  ra_create_s: d=%d s=%d e=%d n=%d\n",d,gs[d],ge[d],n[d]);
    }
...
int ra_declare_s(RARR *arr, IPNT gs, IPNT n)
{
    ...
    for ( d = 0; d < RARR_MAX_NDIM; ++d ) {
      ge[d] = gs[d] + n[d] - 1;
      //      fprintf(stderr,"  ra_declare_s: d=%d s=%d e=%d n=%d\n",d,gs[d],ge[d],n[d]);
    }
...
int ra_print(RARR *arr, FILE* stream)
{
    ...
    case 1:
      start=0;
      while (start<n[0]) {
	for ( li[0] = start; li[0] < iwave_min(start+linelen,n[0]); ++li[0] ) 
...
int ra_size(const RARR *arr, IPNT n)
{
    ...
    for ( d = 0; d < RARR_MAX_NDIM; ++d ) n[d] = arr->_dims[d].n;
...
int ra_a_size(const RARR *arr, IPNT n)
{
    ...
    for ( d = 0; d < RARR_MAX_NDIM; ++d ) n[d] = arr->_dims[d].n0;
...
int ra_gse(const RARR *arr, IPNT gs, IPNT ge)
{
    ...
    // corrected default assignments - WWS 2017.10.23
    for (d=0; d<IARR_MAX_NDIM; d++) {
      gs[d]=0; ge[d]=-1;
    }
...
int ra_gse(const RARR *arr1, const RARR *arr2, IPNT gs, IPNT ge)
{
    ...
    const RARR *arr_max = (arr1->ndim >  arr2->ndim) ? arr1 : arr2;
    const RARR *arr_min = (arr1->ndim <= arr2->ndim) ? arr1 : arr2;
  
    for ( d = 0; d < arr_max->ndim; ++d )
    {
        if ( d < arr_min->ndim)
        {
	    dim = arr_min->_dims + d;
...
int ra_setexchangeinfo(RARR *arr, EXCHANGEINFO *einfo)
{
  ...
  int empty;
	...
	else if ( ndim == 3 )
	{
		...
		err = MPI_Type_create_hvector(n[2], 1, arr->_dims[0].n0 * arr->_dims[1].n0 * sizeof(ireal), einfo->type2, &(einfo->type));		
		if ( err != MPI_SUCCESS )
		{
...
/* private function - do not include in header */
int ra_zero(RARR * a, int tdim) {

  IPNT gs,ge;  /* virtual start, end */
  IPNT tgs,tge;  /* temp virtual start, end */
  IPNT gs0,ge0; /* allocated start, end */
  int err=0;

  if (tdim < 1 || tdim > RARR_MAX_NDIM) return E_BADINPUT;
  ...
  if (a->_s != NULL) {
    ...
    // broken out for vectorization
    if (tdim==1) {
      for (int i=gs[0]; i<=ge[0]; i++) (a->_s)[i-gs[0]] = REAL_ZERO;
    }
    else {
      for (int i=gs[tdim-1]; i<=ge[tdim-1]; i++) {
	tgs[tdim-1]=i;
	tge[tdim-1]=i;
	if (err==0) err=ra_greset(a,tgs,tge);
	if (err==0) err=ra_zero(a,tdim-1);
	if (err==0) err=ra_greset(a,gs,ge);
      }
    }
  }
  return err;
...
/* private overload - do not include in header file */
int ra_ghost_zero(RARR * a, int tdim) {
  ...
  if (a->_s != NULL) {
    ...
    // no point in unrolling 1D case for vectorization,
    // as arrays will be short anyway
    // cycle through subarrays
    for (int i=gs0[tdim-1]; i<gs[tdim-1]; i++) {
      if (tdim==1) (a->_s)[i-gs[0]]=0.0;
      else if (tdim>1) {
	IASN(tgs,gs);
	IASN(tge,ge);
	tgs[tdim-1]=i;
	...
	tge[tdim-1]=i;
	if (err==0) err=ra_greset(a,tgs,tge);
	if (err==0) err=ra_ghost_zero(a,tdim-1);
	if (err==0) err=ra_greset(a,gs,ge);
      }
    }
    if (err) return err;
    
    for (int i=ge[tdim-1]+1; i<=ge0[tdim-1]; i++) {
      if (tdim==1) (a->_s)[i-gs[0]]=0.0;
      else if (tdim>1) {
	IASN(tgs,gs);
	IASN(tge,ge);
...
/* public interface */
int ra_ghost_zero(RARR *a) {
  return ra_ghost_zero(a, a->ndim);
}
...
int ra_a_zero(RARR *arr) {
  ...
  if (ra_a_datasize(arr,&len)) return E_BADINPUT;
...
int ra_a_inner(RARR const * arr1, RARR const * arr2, ireal * ip) {
  ...
  if ( (arr2->_s0 != NULL) && (arr2->_s0 != NULL) ) {
    if (arr1->ndim != arr2->ndim) {
      fprintf(stderr,"ERROR: ra_a_inner\n");
      fprintf(stderr,"  ndims of args not same: %d, %d\n",arr1->ndim,arr2->ndim);
      return E_BADINPUT;
    }
...
int ra_axpy(RARR * arry, RARR const * arrx, ireal a) {
  ...
  if ( (arrx->_s != NULL) && (arry->_s != NULL)) {
    /*
    if (arrx->ndim != arry->ndim) {
      fprintf(stderr,"ERROR: ra_axpy\n");
      fprintf(stderr,"  ndims of args not same: %d, %d\n",arrx->ndim,arry->ndim);
      return E_BADINPUT;
    }
    */
	...
	for (int j=0;j<RARR_MAX_NDIM;j++) {
	  fprintf(stderr,"  nx[%d]=%d ny[%d]=%d\n",j,nx[j],j,ny[j]);
	}
	return E_BADINDEX;
      }
      if (nx0[i]>1) ndimx=i;
