...
ostream & fprint_num_axis(ostream & str, int i, axis a) {
  str<<"axis: n"<<i+1<<"="<<a.n<<" d"<<i+1<<"="<<a.d<<" o"<<i+1<<"="<<a.o<<" id"<<i+1<<"="<<a.id<<"\n";
  return str;
}
...
ostream & fprint_grid(ostream & str, grid a) {
  int i;
  str<<"Grid data structure:\n";
  str<<"gdim = "<<a.gdim<<" axes\n";
  str<<"dim  = "<<a.dim<<" physical axes\n";
  for (i=0;i<a.gdim;i++) fprint_num_axis(str,i,a.axes[i]);
  return str;
}
...
void get_gs(_IPNT gs, grid g) {
  ...
  for (i=0;i<g.gdim;i++) {
    ...
    else gs[i]=(int)(floorf((g.axes[i].o+g.axes[i].d*TOL)/(g.axes[i].d)));
  }
}
...
void get_id(_IPNT id, grid g) {
  ...
  for (i=0;i<g.gdim;i++) 
    id[i]=g.axes[i].id;
  for (i=g.gdim;i<IARR_MAX_NDIM;i++) 
    id[i]=-1;
...
void get_ord(_IPNT od, grid g) {
  ...
  for (j=g.gdim;j<IARR_MAX_NDIM;j++)
    od[j]=j;
}
