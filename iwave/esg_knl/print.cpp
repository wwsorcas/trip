                  ...
                  oRSF &rec_out)
{
    ...
    rec_out.put("n1", static_cast<int>(rec_data[0][0].size()));
    ...
    rec_out.put("o2", static_cast<float>(0));
    rec_out.put("d2", static_cast<float>(1));
    
    rec_out.put("n3", static_cast<int>(3));
    rec_out.put("o3", static_cast<float>(0));
    rec_out.put("d3", static_cast<float>(1));
    
    std::valarray<float> trace(rec_data[0][0].size());
    
    for(int k = 0; k < 3; k++)
    for(int j = 0; j < n_rec; j++)
    {
        for(int i = 0; i < rec_data[0][0].size(); i++)
        trace[i] = rec_data[k][j][i];
