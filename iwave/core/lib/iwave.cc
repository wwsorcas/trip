...
int iwave_construct(IWAVE * state, 
  ...
  */
  ...
  iwave_printf(state,pars,stream,ic);
...
void iwave_printf(IWAVE * state, PARARRAY * pars, FILE * stream, IWaveInfo const & ic) {
  ...
  dump_rs(pars,&(state->model),stream,0,ic,&(state->pinfo));
  dump_rs(pars,&(state->model),stream,1,ic,&(state->pinfo));
}
...
void iwave_dynamic_init(IWAVE * state,
  ...
  for (i=0;i<(state->model).ld_a.narr; i++) {
    ...
    if (fd_isdyn(i,ic)) {
#ifdef VERBOSE
      cerr<<"dynamic_init: zero buffer "<<i<<endl;
#endif
