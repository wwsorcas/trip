...
bool fd_update(int ia, int iv, IWaveInfo const & ic) {
  ...
  if (fd_isdyn(ia,ic) && (iv==ic.get_iwave_fields()[ia].substep)) return true;
  return false;
}
...
int fd_readgrid(PARARRAY * pars, 
  ...
  */
  ...
  for (int io=0; io < ic.get_num_iokeys(); io++) {
	...
	// NOTE THIS ASSUMES THAT PRIMAL GRID IS FIRST!!!! WITH ALL FEATURES!!!
	...
	if (err==0) {
	  ...
	  if ((model->g).gdim == 0) {
	    // 26.04.15	    (model->g).gdim = gtmp.dim;
	    ...
	    // MOD 26.04.15: all spatial axes must come before other axes in axis list.
	    int j = 0;
	    // 26.04.15	    for (int i=0;i<gtmp.gdim;i++) {
	    for (int i=0;i<gtmp.dim;i++) {
	      if ((gtmp.axes[i].id >= 0) && (gtmp.axes[i].id < gtmp.dim)) {
		// 26.04.15		copy_axis(&((model->g).axes[j]),&(gtmp.axes[i]));
		j++;
	      }
	    }
	    if (j != gtmp.dim) {
	      ...
	      fprintf(stream,
		      "  failed to find correct number of spatial axes\n");
	      fprintf(stream,"  = axes with ids between 0 and %d\n",gtmp.dim-1);
	      fprintf(stream,"  in index range [0,%d]\n",gtmp.dim-1);
	      fprintf(stream,"  either %d is not spatial dimension of grid, or\n",gtmp.dim);
	      fprintf(stream,"  axes are not in correct order, with spatial axes\n");
	      fprintf(stream,"  placed before all others\n");
	      return E_BADINPUT;
	    }

	    // 26.04.15: copy whole thing
	    copy_grid(&(model->g),&gtmp);
	  }
	  ...
	  // per id's.
	    ...
	    /*	    
	    if (((model->g).gdim  != gtmp.gdim)) {
	      fprintf(stream,"Error: fd_readgrid\n");
	      fprintf(stream,"  grid gdim = %d from io keyword %s with differs from prototype grid gdim = %d\n",gtmp.gdim, ic.iwave_iokeys[io].keyword.c_str(), (model->g).gdim);
	      return E_BADINPUT;
	    }
	    */	    
	    // loop over id's of spatial axes
	    /* WWS 2017.10.19 this test appears to be incorrect	- should
	       compare corresponding axes, not axes in order of id 
	    ...
	    */
	    ...
	    for (int i=0;i<(model->g).dim;i++) {
	      if (compare_axis((model->g).axes[i],gtmp.axes[i])) {
		fprintf(stream,"Error: fd_readgrid\n");
		fprintf(stream,"  axis %d from io keyword %s\n",
			i, ic.iwave_iokeys[io].keyword.c_str());
		fprint_axis(stream,gtmp.axes[i]);
		fprintf(stream,"  differs from axis %d of prototype grid\n",
			    i);
		fprint_axis(stream,(model->g).axes[i]);
		return E_BADINPUT;
	      }
	    }
	  }
...
int fd_setcompdom(FILE * stream, IPNT cdims, IPNT crank, 
    ...
    //    cerr<<"FD_SETCOMPDOMAIN\n";
    //    ic.write_iwave_fields(cerr);
	...
	*/
	...
	if (fd_isdyn(i,ic)) {
	  ...
	  for (idim = 0;idim < ndim;idim ++) {
	    ...
	    // left physical boundary - remove boundary points
	    if (crank[idim] == 0) {
	      dgs[i][idim]++;
	      if (gtype[i][idim] == DUAL_GRID) dgs[i][idim]--;
	    }
	    // right physical boundary - remove boundary points
	    if (crank[idim] == cdims[idim]-1) dge[i][idim]--;
	  }
	}
	else {
	  /** for static fields, assign for internal extended axes.
	  ...
	  */
	  ...
	  for (idim=0; idim<(model->sfg)[i].gdim; idim++) {
	    ...
	    if ((model->sfg)[i].axes[idim].id > EXTINT-1) {
	      ...
	      dgs[i][idim]=0;
	      //	      dge[i][idim]=g_ge[idim];
	      dge[i][idim]=g_ge[idim]-g_gs[idim];
	    }
	    else {
	      // spatial axes
	      if (((model->sfg)[i].axes[idim].id >-1) && 
