...
#ifdef IWAVE_VERBOSE
  fprintf(stream,"calc_group: ng=%d g=%d first=%d last=%d nrec=%d\n",ng,g,*first,*last,nrec);
  fflush(stream);
#endif
}
...
int traceserver_init(FILE ** fpin, const char * fin,
  ...
  */
  calc_group(first,last,*nrec,stream);
...
#ifdef IWAVE_VERBOSE
  int lsize=retrieveSize();
  fprintf(stream,"traceserver_get lrank=%d lsize=%d\n",lrank,lsize);
  fflush(stream);
#endif
...
#ifdef IWAVE_VERBOSE
  fprintf(stream,"traceserver_get -> bcast\n");
  fflush(stream);
#endif
...
#ifdef IWAVE_VERBOSE
  fprintf(stream,"traceserver_get finish err=%d\n",err);
  fflush(stream);
#endif
...
#ifdef IWAVE_VERBOSE
    fprintf(stream,"traceserver_put: write trace at offset=%ld\n",ftello(fp));
    fflush(stream);
#endif
    fputtr(fp,&(otr->tr));
...
#ifdef IWAVE_VERBOSE
  fprintf(stream,"in init_tracegeom: irec=%d\n",irec);
  fflush(stream);
#endif
  ...
  */
...
#ifdef IWAVE_VERBOSE
  fprintf(stream,"rk=%d rec=%d init_tracegeom <- traceserver_seek recoff=%ld err=%d ntr=%d\n",retrieveRank(),tg->irec,tg->recoff[tg->irec],err,(tg->ntr)[tg->irec]); 
  fflush(stream);
#endif
  if (err) {
...
#ifdef IWAVE_VERBOSE
  fprintf(stream,"init_tracegeom -> loop over trace reads\n");
  fflush(stream);
#endif

  // test data: cannot duplicate tracr, tracl values. most likely
  // error is that these header words are not set - test whether
  // tracr = 0 on 2nd trace, if so assume that tracr is not set for
  // any trace and reset, ditto for tracl. maintain values for these
  // through while loop, initialized here
  // note that SEGY standard specifies that first trace in survey has
  // tracl=1 and first trace in record has tracr=1
  // WARNING: it appears that traceio misuses tracr - it is not the trace
  // number within the record, but the trace number within the SEGY file
  // tracf is the trace number within the record. Since SEGY files and
  // surveys are for all practical purposes the same given the advent of
  // large files, tracr and tracl are the same. This mistake does not affect
  // the use of these headers within traceio, but should be fixed.
  
  int itracl = 1;
  for (int itmp=0; itmp<tg->irec; itmp++) itracl += (tg->ntr[itmp]);

#ifdef IWAVE_VERBOSE
  fprintf(stream,"init_tracegeom: at top of itr loop\n");
  fprintf(stream,"  irec=%d ntr=%d itracl=%d\n",tg->irec,tg->ntr[tg->irec],itracl);
  fflush(stream);    
#endif
  ...
  while ( (itr < (tg->ntr)[tg->irec]) && (!err) ) {
...
#ifdef IWAVE_VERBOSE
    fprintf(stream,"init_tracegeom -> traceserver_get, itr=%d\n",itr);
    fflush(stream);    
#endif
    ...
    if (itr==0) {
      ...
      //      tg->dt   = dt;
#ifdef IWAVE_VERBOSE
      fprintf(stream,"init_tracegeom read header itr=0\n",itr);
      fflush(stream);    
#endif
      /* take trace time data from first trace read */
...
#ifdef IWAVE_VERBOSE
      fprintf(stream,"init_tracegeom: finish reading itr=0 header\n");
      fflush(stream);
#endif
    }
    ...
    if (init[itr]) {
    /* WWS 15.05.16: fix for uninitialized count headers,
       eg. generated from simple madagascar scripts by sfsegywrite:
       count all traces in record, if count > 0 but trace tracr value
       = 0 then overwrite. This will avoid messing up properly
       initialized headers while maintaining a proper count. tracl
       must be initialized to accound for previous records, outside of
       itr loop. */ 
    ...
    if (tg->fldr[tg->ntraces]==0)
      ...
      fprintf(stream,"  global trace indes (itr)=%d local trace index (ntraces)=%d offset=%ld\n",itr,tg->ntraces,otr.m);
      fprintf(stream,"  tracr=%d tracf=%d tracl=%d\n",tg->tracr[tg->ntraces],tg->tracf[tg->ntraces],tg->tracl[tg->ntraces]);
#endif
...
void indexrange(tracegeom const * tg,
  ...
  for (int j=0; j<tg->ndim; j++) {
    rmax[j]=tg->ig[0][j];
    rmin[j]=tg->ig[0][j];
  }

  for (int i=1;i<tg->ntraces;i++) {
    for (int j=0; j<tg->ndim; j++) {
      rmax[j]=iwave_max(rmax[j],tg->ig[i][j]);
      rmin[j]=iwave_min(rmin[j],tg->ig[i][j]);
    }
  }
}
...
void sampletraces(tracegeom * tg,
  ...
  float inc;  /* added 2016.10.09: either 0 or 1, 
  ...
  */
  ...
  for (itr=0;itr<tg->ntraces;itr++) {
    ...
    if (load) {
      /*
#ifdef IWAVE_VERBOSE
      fprintf(stderr,"sampletraces load from %p\n",tg->fpin);
      fprintf(stderr,"  itr=%d it=%d buf=%15.7e ioff=%d ind[0]=%d ind[1]=%d\n",itr,it,(tg->buf)[it+itr*tg->nt],ioff,ind[0],ind[1]);
#endif	
      */
      if (order==0) {
	if (ingrid(ndim,n,gs,ind)) {
	  ...
	  d[ioff]=inc*d[ioff]+fac*(tg->buf)[it+itr*tg->nt];
	}
      }
	...
	if (ndim==1) {
	  ...
	  if (ingrid(ndim,n,gs,ind)) {
	    ...
	    // 	    moff=ind[0]-gs0m[0] + (ind[1]-gs0m[1])*n0m[0];
	    //	    if (m && ingrid(ndim,nm,gsm,ind)) mult*=m[moff]; */
	    d[ioff+n0[0]]=inc*d[ioff+n0[0]] + fac*
	      (1.0-(tg->rg)[itr][0])*
	      (tg->rg)[itr][1]*
	      (tg->buf)[it+itr*tg->nt];
	  ...
	  if (ingrid(ndim,n,gs,ind)) {
	    ...
	    d[ioff+1+n0[0]]=inc*d[ioff+1+n0[0]] + fac*
	      (tg->rg)[itr][0]*
	      (tg->rg)[itr][1]*
	      (tg->buf)[it+itr*tg->nt];
	  ...
	  if (ingrid(ndim,n,gs,ind)) {
	    ...
	    d[ioff+n0[0]*n0[1]]=inc*d[ioff+n0[0]*n0[1]] + fac*
	      (1.0-(tg->rg)[itr][0])*
	      (1.0-(tg->rg)[itr][1])*
	      (tg->rg)[itr][2]*
	      (tg->buf)[it+itr*tg->nt];
	  ...
	  if (ingrid(ndim,n,gs,ind)) {
	    ...
	    d[ioff+1+n0[0]*n0[1]]=inc*d[ioff+1+n0[0]*n0[1]] + fac*
	      (tg->rg)[itr][0]*
	      (1.0-(tg->rg)[itr][1])*
	      (tg->rg)[itr][2]*
	      (tg->buf)[it+itr*tg->nt];
	  ...
	  if (ingrid(ndim,n,gs,ind)) {
	    ...
	    d[ioff+n0[0]+n0[0]*n0[1]]=inc*d[ioff+n0[0]+n0[0]*n0[1]] + fac*
	      (1.0-(tg->rg)[itr][0])*
	      (tg->rg)[itr][1]*
	      (tg->rg)[itr][2]*
	      (tg->buf)[it+itr*tg->nt];
	  ...
	  if (ingrid(ndim,n,gs,ind)) {
	    ...
	    d[ioff+1+n0[0]+n0[0]*n0[1]]=inc*d[ioff+1+n0[0]+n0[0]*n0[1]] + fac*
	      (tg->rg)[itr][0]*
	      (tg->rg)[itr][1]*
	      (tg->rg)[itr][2]*
	      (tg->buf)[it+itr*tg->nt];
	...
	return;
      ...
      /*
#ifdef IWAVE_VERBOSE
      fprintf(stderr,"sampletraces save to %p\n",tg->fpin);
      fprintf(stderr,"  itr=%d it=%d buf=%15.7e ioff=%d ind[0]=%d ind[1]=%d\n",itr,it,(tg->buf)[it+itr*tg->nt],ioff,ind[0],ind[1]);
#endif	
      */
    }
...
int assembletrace(tracegeom const * tg,
  ...
  // nb SEGY standard leaves offset unscaled - x axis only!!!!
  val.i=(int)(tmp-tmps);
  myputhdval(&(otr->tr),"offset",&val);
...
// 22.02.2015 by Yin taper traces 
// added mute by Yin 24.06.2015
void tapermutetraces(tracegeom * tg,
    ...
    if(mwidth>0){
      for (itr=0; itr<tg->ntraces; itr++){
        float smute = mzotime + fabs(mslope*(tg->ig[itr][1]-tg->is[1])*dx);
        float emute = smute + mwidth;
        if ( it <= (smute/tg->dt) ) (tg->buf)[it+itr*tg->nt]=0.f;
        if ( it > (smute/tg->dt) && it < (emute/tg->dt)){
          (tg->buf)[it+itr*tg->nt]*=costap((emute-it*tg->dt)/mwidth);
        }
      }
    }
...
#ifdef IWAVE_VERBOSE
      fprintf(stream,"writetraces: rk 0 write trace %d tracf=%d offset=%ld\n",nb,otr.tr.tracf,otr.m);
      fprintf(stream,"  on unit = %p\n",tg->fpin);
#endif
...
#ifdef IWAVE_VERBOSE
      fprintf(stream,"writetraces, first pass: offset=%ld tracf=%d\n",otr.m,otr.tr.tracf);
      fflush(stream);
#endif
      if (otr.tr.tracf < 1) {
	fprintf(stream,"ERROR: writetraces\n");
	fprintf(stream,"  tracf header word not set\n");
	fflush(stream);
#ifdef IWAVE_USE_MPI
	MPI_Abort(wcomm,err);
#else
...
#ifdef IWAVE_VERBOSE
      fprintf(stream,"RECV from rank %d offset=%ld tracf=%d ns=%d\n",stat.MPI_SOURCE,otr.m,otr.tr.tracf,otr.tr.ns); 
#endif
...
#ifdef IWAVE_VERBOSE
      fprintf(stream,"writetraces, second pass: offset=%ld tracf=%d\n",otr.m,otr.tr.tracf);
      fflush(stream);
#endif
      err=fseeko(tg->fpin,otr.m,SEEK_SET);
      if (err) {
