...
const char * sdoc[] = {
  "green2dt1.x: computes one trace of a 2D point source acoustic green's function in homogeneous",
  "fluid, first term. Field equations are ",
...
int main(int argc, char ** argv) {
  ...
  try {
	...
	tr.data[i] = fac*sqrt((t-r/c)/(t+r/c));
      }
      else {
	tr.data[i]=0.0f;
	//	float s = t+dt - r/c;
	//        tr.data[i] = fac*((4.0f*s/3.0f)-dt)*sqrt(s)/(dt*sqrt(t+r/c));
      }
    }
