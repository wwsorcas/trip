...
The implemented code uses the adjoint state method on the discrete
level, to assure that the computed adjoint operators are adjoint at
the level of machine precision to the computed operators. The issue of
reverse-time storage is resolved through the optimal checkpointing
technique \cite[]{Griewank:book,Symes:06a-pub}, again without loss of
precision.
...
$\|w_r\|\|V^+_{z_s,z_r}\|\|f_s\|$ (thus making the result
dimensionless). The operator norm $\| S^+_{z_s,z_r}\|$ is
