...
The source pulse ($f$ in equation (\ref{eqn:pressure})) is a [2.5, 5, 25, 30]
...
Most descent methods require computed gradient, as input to the
...
%image would be smooth, given the weak constraints on $f$ and $d$, so
%in general it will not define an acceptable update direction for
%velocity. One way out of this predicament is to extend the domain of
%$S$ to a subset of a larger Hilbert space containing
%$C^{\infty}(\bR^2)$, defined by control of finitely many derivatives,
%such as a Sobolev space, and define the adjoint with respect to this
%inner product structure, obtained by concatenating a smoothing
%operator with the formal adjoint. The reader can consult for instance
%\cite{KerSy:94} for more on this idea. For the purposes of this
%discussion we will display the formal gradient (\ref{eqn:gradls})..
...
\plot{velbig_dec_exp}{width=\textwidth}{Example: $v$ = 2 km/s for $z \le
...
\plot{fwiscan}{width=\textwidth}{Plot of values of the FWI objective
  ...
  with the positive average value of the gradient as shown in Figure \ref{fig:fwigrad6kmbighom}. }
