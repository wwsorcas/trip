...
acoustic transpresummission data with single arrivals, AWI implicitly
...
Following the introductory paper on AWI \cite{Warner:16} and much
other work on inverse problems in wave propagation, the framework for
the results reported here is an
active source acoustic model of data generation in Euclidean space
$\bR^3$. Each source is modeled an isotropic point radiator
...
The isotropy and point support assumptions on sources and receivers
are idealizations, and may require relaxation for accurate description of 
actually acquired data. On the other hand, experimentally achievable
data involves finitely many source-receiver paris, as is assumed here:
$X_{sr}$ is finite. 
...
form $f$ through the acoustic system
\begin{eqnarray}
  \label{eqn:awe}
...
The data of the inverse problem to be considered here is the
collection $\{p(\bx_r,t;\bx_s): (\bx_s,\bx_r) \in X_{sr}, t \in \bR\}$ of time
histories or {\em traces} of pressure, recorded by the various source-receiver pairs.
The parameter to be determined is
the bulk modulus $\kappa$.  For simplicity, the material density
$\rho$ is assumed spatially homogeneous, and both $\rho$ and the
source time function $f$ are assumed known.

\subsection{Assumptions}

To make sense of the inverse problem just defined, a number of
additional assumptions are necessary. Some of these are serious
constraints. Prospects for relaxing them will be discussed
later in the paper.

Bulk moduli $\kappa$ are presumed positive, bounded and smooth:
$\log \kappa \in C^{\infty}(\bR^3) \cap L^{\infty}(\bR^3)$. Also, the
source waveform $f$ is supposed smooth, of compact support
$ \subset (0,\infty)$, and the same for every source $\bx_s \in
X_s$. With these assumptions, the symmetric hyperbolic system
\ref{eqn:awe} has a unique distribution solution (for example
\cite{Lax:PDENotes}, Theorem 7.4). The Propagation of Singularities
Theorem (\cite{Tay:81}, Theorem VI.2.1) %and
...
\begin{equation}
  \label{eqn:modop}
  F: \exp(C^{\infty}(\bR^3) \cap
L^{\infty}(\bR^3))  \rightarrow \Pi_{X_{sr}} C^{\infty}(\bR)
\end{equation}
by
\begin{equation}
  \label{eqn:modopdef}
...
In this setting, the inverse problem may be stated as follows: given the set of
source-receiver position pairs $X_{sr}$, source wavelet
$f \in C_0^{\infty}(\bR)$,  density $\rho$, and data traces
$d \in \Pi_{X_{sr}} C^{\infty}(\bR)$, find a bulk modulus $
\kappa \in C^{\infty}(\bR^3) \cap
L^{\infty}(\bR^3)$
...
In the analysis to follow, $M \subset \exp (C^{\infty}(\bR^3) \cap
L^{\infty}(\bR^3))$ denotes an open connected set of bulk moduli $\kappa$
satisfying three further constraints.

Fix an open $\Omega \subset \bR^3$ so that $\Omega \times \Omega$
is a neighborhood of $X_{sr}$. Recall that $\Omega$ is a {\em normal neighborhood} of
$\bx_s \in X_s$ for the sound velocity $c \in C^{\infty}(\Omega)$ if every point $\bx \in \Omega$ is connected to
$\bx_s$ by  a unique ray  (geodesic of the metric $c(x)^{-2}
\sum_{i=1}^3dx_i \otimes dx_i$, \cite{Friedlander:75},
p. 16). In this case, to every point $\bx \in \Omega$ is associated a
unique time of travel (geodesic distance) $\tau(\bx_s,\bx)$ from
$\bx_s$ to $\bx$ (a ``single arrival time'').
\begin{assumption}
  \label{thm:singlearrival} 
  For every $\kappa \in M$, $\Omega\subset \bR^3$ is a normal
  neighborhood of every source point $\bx_s\in X_{s}$ for the sound
  velocity $c=\sqrt{\kappa/\rho}$.
\end{assumption}

\begin{assumption}
  \label{thm:noreturn}
  For every $\kappa \in M$, source point $\bx_s \in X_{s}$, ray ${\bf X}:\bR^+\rightarrow \bR^3$
...
$\Omega$, its subsequent intersection with $\Omega$ is null (hence the
name).

\begin{assumption}
  \label{thm:expdecay}
  For any $n > 0$, there
  exist $\delta, C, t_0 > 0$ that for any $\kappa \in M$, $k$ with $0 \le k \le n$,
  $(\bx_s,\bx_r) \in X_{sr}, f \in C_0^{\infty}(\bR^+)$, the solution
  $(p,\bv)$ of \ref{eqn:awe} satisfies
 \begin{equation}
   \label{eqn:expdec}
    |\partial_t^k p(\bx_r,t;\bx_s)| \le C e^{-\delta t}\|\partial_t
    f\|_{L^2(\bR}, t > t_o
  \end{equation}
\end{assumption}

Since $X_{sr}$ is bounded, the property described here
is a special case of {\em local energy decay}, a much studied
topic. As will be explained in an appendix, Assumption 1 holds if the
rays of geometric optics leave any bounded domain in finite time (the
{\em non-trapping} condition on ray geometry).

The results of this paper concern high frequency, or small wavelength,
asymptotics of the model-data relationship. Since the system
\ref{eqn:awe} is linear and autonomous, the frequency content of the
right-hand side (that is, of $f$) determines the frequency content of
the solutions. The next assumption implies that the Fourier transform
of $f$ vanishes to second order at zero frequency, and guarantees that
$f$ is at least somewhat oscillatory.

\begin{assumption}
  \label{thm:zerofrequency}
  There exists $\tilde{f} \in
  C_0^{\infty}(\bR^+)$ so that
  \begin{equation}
    \label{eqn:zerofreq}
    f = \partial_t^2 \tilde{f}.
  \end{equation}
\end{assumption}

{\bf Remark:} Of the first four assumptions, the last three can be
weakened to some extent. Assumptions \ref{thm:noreturn} and
\ref{thm:expdecay} ensure that regions in $\bR^3$ far from $\Omega$ do
not influence the data, at least in high frequency limit. At the cost
of more involved arguments, the same assurance follows from truncating
the data in time. Assumption \ref{thm:zerofrequency} is one of many
ways to focus on the high frequency limit. However, as we shall show
in the final section of this paper, the first assumption is essential:
if multiple geodesics connect sources and receivers, no result of the
type described in Theorem \ref{thm:main} below can possibly hold.

The asymptotic analysis explained in the sequel is
based on a family of source waveforms, generated from a reference
waveform $f_1 \in C_0^{\infty}(\bR^+)$. For $\lambda \in (0,1]$, define
\begin{equation}
  \label{eqn:wvltfam}
  \fl(t)= \lambda^{\frac{1}{2}}f_1\left(\frac{t}{\lambda}\right).
\end{equation}
The dimensionless scale parameter $\lambda$ is a proxy for
wavelength in the arguments that follow. The precise relationship
explained below.

The reference waveform $f_1$ is presumed to satisfy Assumption
\ref{thm:zerofrequency}, that is, 
\[
  f_1 = \partial^2 \tilde{f}_1
\]
Define the related family $\tilde{f}_{\lambda}$ by
\begin{equation}
  \label{eqn:tildeflam}
  \tilde{f}_{\lambda}(t) =
  \lambda^{5/2}\tilde{f}_1\left(\frac{t}{\lambda}\right).
\end{equation}
All members of $\{\fl: \lambda \in (0,1]\}$ then satisfy Assumption
\ref{thm:zerofrequency}: 
\[
  \partial^2 \tilde{f}_{\lambda} = f_{\lambda}.
\]

Denote by $\Fl$ the modeling operator $F$ defined by the choice $f=\fl$.

\subsection{Full Waveform Inversion}

Suppose that Assumption \ref{thm:expdecay} holds. Then, each trace $p(\bx_r,\cdot;\bx_s)$ is in $L^{p}(\bR)$ for $p
\ge 1$.
Denote by $L^p(X_{sr} \times \bR)$ the product space $l^p(X_{sr},L^p(\bR))$
with the usuall product norm. 

Accordingly, acoustic FWI may be posed as a nonlinear least-squares
...
\end{equation}
Many practical implementations of FWI have been based on minimization
of $J_{\rm FWI}$, often with the addition of regularization terms,
constraints, and preprocesses that enhance the chances of obtaining a
global minimizer via local optimization (descent) methods.  Indeed,
discretizations of the components of \ref{eqn:fwi} result in
large-scale computational tasks, so that only local optimization
algorithms related to Newton's method are feasible for minimization of
$J_{\rm FWI}$. However, such
methods tend to stagnate far from global
minimizers if the initial choice of $\kappa$ does not predict the
arrival times of waves correctly within a half-wavelength or so
\cite[]{GauTarVir:86,VirieuxOperto:09}. While it is widely believed
that this stagnation occurs at local minima far from global
minimizers, such local minima have not been shown to exist, except in
low-dimensional specializations.

This paper does not prove existence of non-global minima. It does
however establish a property of $J_{\rm FWI}$ that is consistent with
their existence.

\begin{theorem}
  \label{thm:fwi}
  Suppose that Assumptions 1 - 5 hold
  for $\kappa \in exp{M}$, and that that
as  $\dl$ denotes its family of noise-free data, that is,
  \[
    \dl = \Fl[\kappa], \, \lambda > 0.
  \]
  Then there exist positive $K^*, L_*<L^* \in \bR_+$ and
  perturbations $\kappa_{\lambda} \in exp(M), \lambda > 0,$ so that
  \[
   L_* \le  J_{\rm FWI}[\kappa_{\lambda};\dl] \le L^*, \,\lambda > 0
  \]
  but
  \[
    \|\kappa_{\lambda} - \kappa\|_{L^{\infty}(\bR^3)} \le K^*\lambda.
\]
\end{theorem}

Since $ J_{\rm FWI}[\kappa;\dl]=0$, the modulus of continuity of
$J_{\rm FWI}[\cdot;\dl]$ is bounded below by a multiple of $1/\lambda$
as $\lambda \rightarrow 0$. This observation does not itself
imply that $ J_{\rm FWI}$ oscillates, or has stationary points other
than its global minimizer, but is consistent with such behaviour, and
suggests that local optimization methods would face could be expected
to perfore arbitrarily poorly as the data wavelength becomes small.
...
Here
\begin{itemize}
  \item[i. ]$T$ is the self-adjoint extension of the operator
$Tu(\bx_r,t;\bx_s) = tu(\bx_r,t;\bx_s)$, $u \in \Pi_{(\bx_s,\bx_r) \in
  X_{sr}}{\cal S}(\bR)$, to $L^2(X_{sr} \times \bR)$, with domain
$D(T)$;
\item[ii. ] $u_{\sigma}[\kappa,d]$ is the solution of
\begin{equation}
  \label{eqn:reg}
...
problem \ref{eqn:reg},
it is not immediiately obvious that $u_{\sigma}[\kappa;d] \in D(T)$,
so the definition \ref{eqn:awidefmain} requires justification.

The main result of this analysis is

\begin{theorem}
  \label{thm:main}
  Suppose that Assumptions 1 - 5 hold. For $0 <
  \lambda \le 1$, set
  ...
  \item[1. ] for $(\bx_s,\bx_r) \in X_{sr}$, $\log \kappa \in M$, and
    $\sigma, \lambda >0$, $u_{\sigma}[\kappa;\dl] \in
    D(T)$;
...
Unless otherwise stated, from now on it is presumed that assumptions
\ref{thm:expdecay}, \ref{thm:singlearrival}, \ref{thm:noreturn},
and \ref{thm:zerofrequency} hold for $\kappa, X_{sr} \subset \Omega$,
  and $f$. the letter $C$, or sometimes $C_k,
k=0,1,2,...$ or
$C_*,C^*$, will designate a constant (or constants) that are uniform
for configurations of $\kappa, \Omega, X_{sr}$, and $f$ for which the
constants appearing in these assumptions are uniform. 
...
second-order wave equations. Introduce the acoustic potential $q$:
\begin{equation}
  \label{eqn:acpo}
  q (\bx,t;\bx_s)  = \rho \int_{-\infty}^t ds \, p(\bx,s;\bx_s).
\end{equation}
From the second equation in \ref{eqn:awe}, $\bv = \nabla q$, and it follows from \ref{eqn:awe} that 
\begin{equation}
  \label{eqn:awe2}
  \frac{\rho}{\kappa} \partial_t^2 p - \nabla^2 p = \rho \partial_t f\delta(\bx-\bx_s), \nonumber \\
  q=0, \, t \ll 0.
\end{equation}
Conversely, a solution $q$ of the system \ref{eqn:awe2} generates a
solution $(p,\bv)$ of \ref{eqn:awe}. 
...
The construction described by \cite{Friedlander:75} pertains to a
normal neighborhood, as asserted by Assumption \ref{thm:singlearrival}
for $X_{s} \subset \Omega$. The construction produces
functions $\tau[\kappa]$ (travel time), $a[\kappa]$
(geometric amplitude) on $X_s \times \Omega$ and $b[\kappa]$
(remainder kernel) on $X_s \times \Omega \times \bR$. The travel time $\tau$
and amplitude
$a$ are smooth and positive off the
diagonal - that is, for each $\bx_s \in \Omega$, $\tau[\kappa](\bx_s,\cdot) \in C^{\infty}(\Omega \setminus
\{\bx_s\})$, and similarly for $a[\kappa]$.

The construction explained in \cite{Friedlander:75} defines
$b[\kappa](\bx_s, \bx_r,t)$ for small positive $t$; it vanishes for
$t<0$, and has jump discontinuities in derivatives at $t=0$ determined
by solution of a sequence of transport equations. The traveltime
$\tau$ satisfies the eikonal equation
\[
  \sqrt{\frac{\kappa}{\rho}} |\nabla \tau(\bx_s,\cdot)| = 1 \mbox{ in
  } \Omega \setminus \{\bx_s\}, \, \lim_{\bx \rightarrow \bx_s}
  \tau(\bx_s,\bx) = 0.
\] 
A by-product of the construction are bounds on $a$ and $b$, depending on
derivatives of $\kappa$ and the geometry of geodesics.

The upshot is the Hadamard decomposition of the Green's function
...
Without further assumptions, this decomposition holds within the
normal neigborhood $\Omega$ and within a small time interval about the
light cone, that is, for small $|t-\tau(\bx_s,\bx)|$. The No-Return
assumption, that no bicharacteristics (geodesics) return to the domain
$\Omega$ once having left it, combines with the previously cited
theorem on Propagation of Singularities to assure smoothness of
Green's function for a source at $\bx_s \in X_s$ and
$t>\tau(\bx_s,\bx)$. That is, the remainder $b$ can be extended to
$t > 0$ as the smooth part of the fundamental solution. The extended
remainder depends on the global behaviour of $\kappa$ and the
bicharacteristic geometry that it generates.

Given a waveform $f \in C^{\infty}_0(\bR)$, define for $\log \kappa
\in C^{\infty}(\bR^3)$ satisfying Assumptions \ref{thm:singlearrival}
and \ref{thm:noreturn},
\begin{eqnarray}
  \label{eqn:fdefs0}
  F^0[\kappa] = a[\kappa] \delta_{\tau[\kappa]} * \partial_t f \\
    \label{eqn:fdefs1}
  F^1[\kappa] = b[\kappa]H*\delta_{\tau[\kappa]} * \partial_t f
\end{eqnarray}
so that $F[\kappa] = F^0[\kappa] + F^1[\kappa]$. 
...
\begin{prop}
  ...
  \begin{eqnarray}
    \label{eqn:f0bnd}
    \|F^0[\kappa]\|_{L^1(X_{sr} \times \bR)} \le C\|\partial_t 
    f\|_{L^1(\bR)}\\
    \label{eqn:f1bnd}
    \|F^1[\kappa]\|_{L^1(X_{sr} \times \bR)} \le C\| f\|_{L^1(\bR)}
  \end{eqnarray}
  Also, there exist $C, \delta, t_0>0$, so that for $t \ge t_0, (\bx_s,\bx_r)
  \in X_{sr}$,
  \begin{equation}
    \label{eqn:f1asymp}
    |F^1[\kappa](\bx_r,t;\bx_s)| \le C e^{-\delta t}\| f\|_{L^1(\bR)}
  \end{equation}
\end{prop}

\begin{proof}
The first conclusion follows from the first definition \ref{eqn:fdefs0} and
Young's inequality. 
From the second definition \ref{eqn:fdefs1},
\[
  F^1[\kappa](\bx_r,t;\bx_s) =
  b[\kappa](bx_r,\tau[\kappa](\bx_s,\bx_r);\bx_s)
  f(t-\tau[\kappa](\bx_s,\bx_r))
\]
\begin{equation}
  \label{eqn:f1alt}
  + (\partial_t
b[\kappa](\bx_r,\cdot;\bx_s)*H(\cdot)*f(\cdot))(t)
\end{equation}
This identity implies that for $t_0$ as specified in Assumption \ref{thm:expdecay},
\begin{equation}
  \label{eqn:f1nonasymp}
  \|F^1[\kappa](\bx_r,\cdot;\bx_s\|_{L^1[0,t_0]} \le C\|f\|_{L^1(\bR)}.
\end{equation}
with suitable choice of $C>0$.
  
Note that for $t \ge \tau(\bx_r,\bx_s) + \max({\rm supp }f)$,
$F^1[\kappa](\bx_r,t;\bx_s) = F[\kappa](\bx_r,t;\bx_s) = p[\kappa](\bx_r,t;\bx_s)$.
Recall the relation $p =
\rho \partial_t q$ with the potential $q$ (solution of the second
order wave equation \ref{eqn:awe2}).

Invoke Assumption \ref{thm:zerofrequency} for the waveform $f$, that is,
$f=\partial^2\tilde{f}$ with $\tilde{f} \in C_0^{\infty}(\bR^+)$. Define
$\tilde{q}$ to be the solution of the \ref{eqn:awe2} with $f$ replaced by
$\tilde{f}$, and $\tilde{p} = \partial_t \tilde{q}$ the corresponding
pressure. According to Assumption \ref{thm:expdecay},
\begin{equation}
  \label{eqn:ptildebnd}
  |\partial^2 \tilde{p}(\bx_r,t;\bx_s)\| \le C e^{-\delta t} \|\partial_t
  \tilde{f}\|_{L^2(\bR)},\,t >t_0
\end{equation}
for suitable $C, \delta, t_0> 0$.

Set $t_1=\inf \mbox{supp }\tilde{f}, t_2 = \sup  \mbox{supp
}\tilde{f}$, so that
\[
\|\partial_t \tilde{f}\|_{L^2(\bR)}^2 = \int_{t_1}^{t_2} \,dt
\left|\int_{t_1}^t\,f\right|^2 \le C\|f\|_{L^1(\bR)}^2.
\]
Since $p = \partial_t^2 \tilde{p}$, this
inequality and \ref{eqn:ptildebnd} imply inequality \ref{eqn:f1asymp},
possibly with larger $C, t_0$ and smaller $\delta > 0$. Combine
\ref{eqn:f1asymp} with 
inequatliy \ref{eqn:f1nonasymp} to conclude that $F^1$ is absolutely
integrable, that is, conclusion \ref{eqn:f1bnd}.
\end{proof}

%\subsection{Exponential Decay}
%The Exponential Decay condition follows from Hadamard's decomposition
%and result of \cite{Vainberg:1982} (see also \cite{Hristova:09},
%Theorem 2, \cite{EgorovShubin}) states that local energy decays
%exponentially provided that the system \ref{eqn:awe} (in effect, the
%coefficient $\kappa$) satisfies the{\em non-trapping} condition. The
%system is non-trapping if all rays of gemetric acoustics, that is,
%geodesics of the Riemannian metric associated to the system
%\ref{eqn:awe}, tend to infinity. More precisely, for any
%$R > r>0$ and $\log \kappa \in M$, there exists $T>0$ so that any
%geodesic ${\bf X}:\bR^+\rightarrow \bR^3$ with ${\bf X}(0) \in B_r(0)$
%satisfies $|{\bf X}(t)|>R$ for $t > T$. Vainberg's result is: 
...
\begin{equation}
  ...
  S[\kappa]u = u*F[\kappa]
\end{equation}
Under the assumptions of Proposition \ref{thm:fbounds}, $F[\kappa] \in
L^1(X_{sr} \times \bR)$. so $S[\kappa]$ is bounded on $L^2(X_{sr} \times \bR)$.
$S$ represents an
...
\begin{prop}
  ...
  \end{equation}
  Also, the least squares problem \ref{eqn:regs} with $\sigma > 0$ and
  $d\in L^2(X_{sr} \times \bR)$ has a unique solution
...
\end{prop}
\begin{proof}
  The first conclusion follows from Proposition \ref{thm:fbounds} and
  Young's inequality. For $\sigma > 0$, the Lax-Milgram Theorem
  (\cite{Yosida}, section III.7) assures the existence of unique
  minimizer $u_{\sigma}[\kappa;d]$ satisfying the normal equation
\begin{equation}
  \label{eqn:normal}
...
In the discussion to follow, $\kappa$ is the argument of $S$ and
$u_{\sigma}$ throughout, so drop it temporarily from the notation. The
expressions to follow pertain to each trace, that is, for each
$(\bx_s,\bx_r) \in X_{sr}$, so drop these from the notation also.

\begin{lemma}
  \label{thm:tcomm}
  Suppose $g \in {\cal S}$. Then for $\phi \in {\cal S}$,
  \[
    [T, g*] \phi= (Tg)*\phi.
  \]
\end{lemma}
\begin{proof}
  For $\phi \in {\cal S} \subset D(T)$,
  \[
    T g*\phi (t) - g* T\phi(t) = \int\,ds \, (t g(t-s)\phi(s) -
    g(t-s)s\phi(s)) = \int\, ds \, (t-s)g(t-s)\phi(s) = (Tg *) \phi
    (t).
  \]
\end{proof}
...
\begin{lemma}
  \label{thm:commbnds}
  The commutators $[T,S^0]$ and $[T,S^1]$ are bounded, and
  \begin{eqnarray}
    \label{commbnd0}
    \|[T,S^0]\| &\le & C\|\partial_t f\|_{L^1(\bR)}\\
    \label{commbnd1}
    \|[T,S^1]\| &\le & C\|f\|_{L^1(\bR)}
  \end{eqnarray}
\end{lemma}
\begin{proof} $S^0$ and $S^1$ are convolutions with $F^0$ and $F^1$
  respectively, both members of ${\cal S}$. Thus $[T,S^0] = (TF^0)*$,
  and $(TF^0)(t) =  a t \partial_t f(t-\tau)$.
  Since the range of $\tau$ over $X_{sr}$ is bounded, and $f$ has
  compact support, $TF^0$ is  bounded in $L^1(\bR)$ by
  $C\|\partial_t f\|_{L^1(\bR)}$. From the identity \ref{eqn:f1alt},
  \[
    \|tF^1\|_{L^1([0,t_0])} \le C t_0 \|f\|_{L^1(\bR)}
  \]
  From \ref{eqn:f1asymp}, $tF^1\le Ce^{-\delta t}\|f\|_{L^1(\bR)}$ for
  $t>t_0$, with adjusted $\delta$ and $C$.  Combining these two
  observationss, conclude that actually $tF^1 \in {\cal S}$ and
  $\|TF^1\|_{L^1(\bR)} \le \| f\|_{L^1(\bR)}$, establishing the second bound.
\end{proof}

\begin{lemma}
  \label{thm:rhsbnd}
  Suppose $d \in D(T)$. Then $S^T d \in D(T)$, and
  \begin{equation}
    \label{eqn:rhsbnd}
    \|TS^Td\| \le C\|\partial f\|_{L^1(\bR)} (\|d\| +
    \|Td\|).
  \end{equation}
  
\end{lemma}
\begin{proof}
  Choose $\phi \in {\cal S} \subset D(T)$. From Proposition
  \ref{thm:fbounds}, $S^Td\| \le C\|\partial_t f\|_{L^1(\bR)}\|d\|$. Then
  \[
    \langle T\phi, S^Td \rangle = \langle ST\phi,d\rangle = \langle
    (TS + [S,T])\phi, d\rangle 
  \]
  From the previous lemma, $\|[S,T]\| \le C\|\partial_t
  f\|_{L^1(\bR)}$, so the above is
  \[
    \le C\|\phi\| \|\partial_t f\|_{L^1(\bR)}(\|Td\| + \|d\|)
  \]
...
\begin{lemma}
  \label{thm:invest}
  Suppose that $\phi \in {\cal S}$ and $\psi \in L^2(\bR)$ so that
  \[
    (S^TS + \sigma I)\psi = \phi.
  \]
  Then $\psi \in {\cal S}$, and $\|\psi\| \le \frac{C}{\sigma}\|\phi\|$.
\end{lemma}
\begin{proof}
  From Proposition \ref{thm:fbounds}, $F \in {\cal S}$. as is its
  Fourier transform $\hat{F}$. The Fourier transforms of $\phi$ and
  $\psi$ are related by
  \[
    (\bar{\hat{F}}\hat{F} + \sigma)\hat{\psi} = \hat{\phi}.
  \]
  Since the first summand on the LHS is non-negative real,
 $ |\hat{\psi}| \le \sigma^{-1}|\hat{\phi}|$, whence $\hat{\psi}$
 hence $\psi$ are both in {\cal S}.
 \end{proof}
   
\begin{prop}
  \label{thm:tl2}
  Suppose that $d \in D(T)$. Then
  $u_{\sigma}[\kappa;d](\bx_r,\cdot;\bx_s)  \in D(T)$ for all
  $(\bx_s,\bx_r) \in X_{sr}$ and
  \[
...
\begin{proof}
  ...
  SInce these arguments do not vary, drop $[\kappa,d]$, $\bx_r$ and $\bx_s$ from the
  notation for the filter $u_{\sigma}$ and other quantities.
  For $\phi \in {\cal S}$, let $\psi \in {\cal S}$ so that
  $(S^TS + \sigma I)\psi = \phi$.
  \[
    \langle T\phi, u_{\sigma}\rangle = \langle T (S^TS +
    \sigma I)\psi, u_{\sigma}\rangle
  \]
  \begin{equation}
    \label{eqn:inter1}
    =\langle T \psi, S^T d \rangle + \langle [T, S^TS] \psi, u_{\sigma}
    \rangle
  \end{equation}
  Note that
  \[
    [T,(S^0)^TS^0] = [T, (a (-\partial_t\check{f})*
    \delta_{-\tau})*(a \partial_tf *\delta_{\tau})* ]
  \]
  \[
   = a^2[T,
   ( (-\partial_t \check{f})*\partial_tf)*] = -a^2
    ((T \partial_t \check{f})*(\partial_tf)* + \partial_t \check{f}*(T\partial_tf)*)
  \]
  so
  \[
    \|[T,(S^0)^TS^0]\| \le C \|\partial_t f\|_{L^1(\bR)}\|T\partial_t
    f\|_{L^1(\bR)}
  \]
  Using the inequalities from Lemmas \ref{thm:fbounds} and \ref{thm:commbnds},
  \[
    \|[T, (S^0)^TS^1]\| \le \|[T,(S^0)^T]S^1\| + \|(S^0)^T [T,S^1]\|
  \]
  \[
    \le C(\|\partial f\|_{L^1(\bR)}\|f\|_{L^1(\bR)}.
  \]
  Similarly,
  \[
    \|[T, (S^1)^TS^0]\| \le C(\|\partial
    f\|_{L^1(\bR)}\|f\|_{L^1(\bR)},
  \]
  and
  \[
    \|[T, (S^0)^TS^0]\| \le C\|f\|_{L^1(\bR)}^2.
  \]
  Combining the last several inequalities, obtain
  \begin{equation}
    \label{eqn:TScomm}
    \|[T, S^TS]\| \le C\|\partial_t
    f\|_{L^1(\bR)}(\|f\|_{L^1(\bR)}+\|T\partial_t f\|_{L^1(\bR)}).
  \end{equation}
  Use this inequality and inequality \ref{eqn:rhsbnd} to estimate the RHS
  of equation \ref{eqn:inter1}:
  \[
    |\langle T\phi, u_{\sigma}\rangle| \le C\|\psi\|\|\partial_t
    f\|_{L^1(\bR)}(\|d\| + \|Td\|) + \|\partial_t
    f\|_{L^1(\bR)}(\|f\|_{L^1(\bR)}+\|T\partial_t
    f\|_{L^1(\bR)})\|u_{\sigma}\|)
  \]
  Use \ref{eqn:unorm} to estimate $u_{\sigma}$, the estimate in Lemma
  \ref{thm:invest} to estimate $\psi$, and rearrange to obtain
  \[
    |\langle T\phi, u_{\sigma} \rangle| \le
  \]
  \[
    \frac{C}{\sigma}\left(\left(
        \frac{\|\partial_t f\|^2_{L^1(\bR)}}{\sigma}(\|f\|_{L^1(\bR)}+\|T\partial_tf\|_{L^1(\bR)})
      \right)\|d\| + \|\partial_t  f\|_{L^1(\bR)}(\|d\|
      +\|Td\|)\right)
    \|\phi\|.
  \]
...
The exception is the single-arrival hypothesis: it is
...
Linear elasticity is a much better model for mechanical waves
