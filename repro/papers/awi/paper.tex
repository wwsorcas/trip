...
collection $\{p(\bx_r,t;\bx_s): (\bx_s,\bx_r) \in X_{sr}\}$ of time
records or {\em traces} of pressure, recorded by the various source-receiver pairs.
The parameter to be determined is
the bulk modulus $\kappa$.  For simplicity, the material density
$\rho$ is assumed spatially homogeneous and known.
...
The bulk modulus $\kappa$ is presumed positive, bounded and smooth: $\log
\kappa \in M$, where $M$ is a bounded subset of $C^{\infty}(\bR^3) \cap
L^{\infty}(\bR^3)$. Also, the source waveform $f$ is supposed smooth,
of compact support, and the same for every source $\bx_s \in X_s$. 
Then the symmetric hyperbolic system \ref{eqn:awe}
...
The second and third assumptions explicitly involve constraints on ray geometry
...
asymptotics of the model-data relationship. The asymptotic analysis is
based on a family of source waveforms, generated from a reference
waveform $f_1 \in C_0^{\infty}(\bR)$. For $\lambda > 0$, define
\begin{equation}
  \fl(t)= \lambda^{\frac{1}{2}}f_1\left(\frac{t}{\lambda}\right).
\end{equation}
The dimensionless scale parameter $\lambda$ is a proxy for
wavelength in the arguments that follow. The precise relationship explained below. 

Denote by $\Fl$ the modeling operator $F$ defined by the choice $f=\fl$.

\subsection{Full Waveform Inversion}

Suppose only the Exponential Decay hypothesis is assumed. Then, each trace $p(\bx_r,\cdot;\bx_s)$ is in $L^{p}(\bR)$ for $p
\ge 1$.
...
\end{equation}
Practical implementations of FWI may include regularization terms,
constraints, and preprocesses that enhance the chances of success. 

Discretizations of the components of \ref{eqn:fwi} yields
computationally large problems, for which only local optimization
methods are feasible. However, optimization algorithms related to
Newton's method, applied to $J_{\rm FWI}$, tend to stagnate far from
global minimizers if the initial choice of $\kappa$ does not predict
the arrival times of waves correctly within a half-wavelength or so
(the meaning of ``wavelength'' in this context will be explained
below) \cite[]{GauTarVir:86,VirieuxOperto:09}. While it is widely
believed that this stagnation occurs at local
minima far from global minimizers, such local minima have not been
shown to exist, except in low-dimensional specializations.
...
\begin{theorem}
  \label{thm:fwi}
  Suppose that Exponential Decay, Single Arrival and No Return hold
  for $\kappa \in exp{M}$, and that that
  $\dl$ denotes its family of noise-free data, that is,
  \[
