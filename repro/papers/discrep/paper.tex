...
squares FWI, based on an extension of the model-to-data mapping.  We
show exactly how an example of this {\em extended inversion} approach
leads to solution of a very simple acoustic transmission
inverse problem {\em via} local gradient-based optimization. Despite
its simplicity, this problem exhibits several important
characteristics of all inverse problems in wave propagation in which
wave velocities are amongst the unknowns. It has the advantage of
permitting explicit calculations that reveal clearly the failure
mechanism for FWI, that is, straightforward nonlinear least squares,
and also the reasons for success of an extended inversion approach.

In the remainder of this introduction, we will briefly describe the conceptual
framework of this inverse problem, and sketch our main results. We leave out
some technical details that are supplied in the next section, where we give
precise statements of our results. In particular, theorems referenced
in the rest of this section are to be found, stated in full, in the
next section.

The physical setting for our discussion is the propagation of waves through a
...
shift. This time shift operator is the basis of many illustrations of
...
$\|F[m]w-d\| \le \epsilon\|d\|$ (Problem Statement
...
\begin{eqnarray}
  ...
  e[m,w;d] & = & r[m,w;d]^2, \label{eqn:mserr}\\
...
of $r$, $e$, $\alpha^2 g$, and $\Ja$ nondimensional.

The only obvious requirement on the penalty operator $A$ is that it
penalize 
...
few details not discussed in the preceding sketch. Then we provide a
discussion of the potential theoretical and practical import of these
results, including relation with prior work, some numerical
...
the time dependence of the point source intensity (``wavelet'' or
``source wavelet'') at location
$\bx=\bx_s$. Then the (excess) pressure field $p(\bx,t)$ obeys a
scalar wave equation \cite[]{Frie:58}:
\begin{eqnarray}
  \label{eqn:awe}
  ...
  p(\bx,t)&=&0, t\ll 0
\end{eqnarray}
Suppose that the pressure at a receiver position $\bx=\bx_r$ is recorded, at distance $r>0$ from the
source position $\bx_s$. We shall refer to the resulting function of
time as a {\em trace}, from common seismic usage. The dominant information in a single
...
Define $r=|\bx_r-\bx_s| $ to be the source-receiver distance
(``offset''). The predicted signal at
...
in \ref{eqn:mod}, which we repeat here for convenience:
\begin{equation}
\label{eqn:mod1}
...
hypothetical single trace data.
...
$m$. A more precise version of the constraint proposed in the
introduction involves a selection of a maxium support radius
...
extended inversion is the reduction of the penalty objective $\Ja$
...
This phenomenon - nondifferentiable objective, differentiable reduced
objective - is replicated for a wide variety of extended
...
The inverse problem stated at the beginning of this paper concerns
three types of
objects: a set of vectors of measured or observed parameters (``the
data''); a set of vectors of non-observed parameters (``the model''); and a
modeling or prediction operator, which accepts model vectors as
input and produces predicted data vectors as output.

The ``inverse problem'' is really a family of questions, parametrized
by a scalar tolerance. The question implicitly posed in the statement
\ref{eqn:probstat0} is: given a tolerance and an observed data vector,
is there a model vector for which the predicted data vector is within
the tolerance of the observed data vector, in the sense of a distance
function on the collection of all possible data vectors.

This deterministic definition of inverse problems has been presented by
many authors, for instance \cite{ddjackson:72,Parker:94}. Most variants 
presume that the data and model vectors form Hilbert spaces, or
possibly subsets of Hilbert spaces, and that the distance between
predicted and observed data vectors is the norm of the difference. Our
approach fits squarely in this tradition.

In any concrete instance, a ``yes'' answer can only be justified by
exhibition of a model vector satisfying the fit criterion, hence the
form of the statement \ref{eqn:probstat0}. The extension approach
discussed in this paper relies on use of an extended modeling or
prediction operator, satisfying two criteria:
\begin{itemize}
\item the extended operator is an extension of the modeling
  operator, that is, its domain is a superset of the model vector
  space and, when fed a model vector, it replicates the action of
  the modeling operator;
\item the extended operator is {\em surjective}, that is, all data
  vectors can be fit without error.
\end{itemize}
...
\begin{proof} of Theorem \ref{thm:fwi}: From the definition \ref{eqn:mod},
\[
...
\begin{proposition}
  ...
  e[m,\aw[m,d];d] = \frac{1}{2}\int_{t_{\rm min}}^{t_{\rm max}} \,dt\,(4\pi r \alpha a(t-mr))^4(1 +
  (4\pi r \alpha a(t-mr))^2)^{-2}d(t)^2
\end{equation}
\begin{equation}
  \label{eqn:anninormgen}
  p[m,\aw[m,d];d] = \frac{1}{2}\int_{t_{\rm min}}^{t_{\rm max}} \,dt\,(4\pi r a(t-mr))^2(1 +
  (4\pi r \alpha a(t-mr))^2)^{-2}d(t)
\end{equation}

\begin{equation}
  \label{eqn:expjgen}
\tJa[m;d] = \frac{1}{2}\int_{t_{\rm min}}^{t_{\rm max}}\,dt\,(4\pi r \alpha a(t-mr))^2(1+(4\pi r \alpha 
a(t-mr))^2)^{-1}d(t)^2. 
\end{equation}
\end{proposition}
...
\begin{corollary}
  ...
  Assume the hypotheses of Proposition \ref{thm:norminvexp}, item 3. Then 
\begin{equation}
  \label{eqn:residnorm}
  e[m,\aw[m,d];d] 
...
\noindent {\bf Remark:} This is the only error bound in our treatment
of this problem in which the size of $\alpha$ plays a role. It
suggests that the estimate of $m$ becomes more accurate in the
presence of noise for larger $\alpha$. This result is intuitively
