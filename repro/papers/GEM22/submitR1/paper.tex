...
pressure source
$h_r$ on $\Sigma_r$ and output $P_ss\hat{p}$ on $\Sigma_s$. To
...
\]
Differentiate, replace time derivatives by space derivatives and
...
\begin{equation}
  ...
  {\cal S}^T(h_r,f_r) = -(P_s\hat{p},P_s(\hat{\bv}\cdot{\bf
    n}_s))^T.
\end{equation}
Speclializing to the pressure modeling operator $S$ (equation
\ref{eqn:sdef}),
\begin{equation}
  \label{eqn:padj}
