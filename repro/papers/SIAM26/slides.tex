...
C_0^{\infty}(\bR)$ {\color{blue} known}

Predicted pressure data $p[\kappa](\bx_r,t;\bx_s)$

  {\color{blue} Dynamic Inverse Problem}: given data
  $\{d(\bx_r,\cdot;\bx_s): (\bx_s,\bx_r) \in X_{sr}\}$, seek $\log \kappa \in
  C^{\infty}(\bR^3)$ so that $p[\kappa]
  \approx d$.
...
$x_s=3000$ m, 181 receiver positions ($\bx_r=(x_r,z_r)$, blue) at $x_r=5000$ m. Right:
...
Conventional remedy for cycle skipping: pick times $t(\bx_r,\bx_s)$,
minimize $J_{\rm TT}$, initiate minimization of $J_{\rm FWI}$
from result, low $\rightarrow$ high frequency continuation -
widespread use, much success (including commercial)

Still, many attempts at alternatives: is there a replacement for FWI
that uses waveform data directly but does not cycle-skip, implicitly
accomplishes travel-time inversion?
}
...
\frame{
  ...
  {\color{blue} Adaptive Waveform Inversion}: Since $t\delta(t) =0$,
...
\frame{
  ...
  \[
    ...
    \int_{\tau(\bx,\bx_s)}^t ds\,b(\bx,s;\bx_s)f(t-s)
  \] 
  $a$, $b$ smooth for $\bx \ne \bx_s$ - ``No Return'' $\Rightarrow$
  global in $t$ 

  So $p[\kappa](\bx_r,t;\bx_s) = \rho a(\bx_r,\bx_s) \partial_t f(t-\tau(\bx_r,\bx_s)) + ...$

  (Hadamard 1923, Courant \& Hilbert '62, Friedlander '75, H\"{o}rmander '85, Wei et al. '24)
}

\frame{
  ignore``...'', noise free data $d^* = p[\kappa^*] \approx
  \rho a^*f(t-\tau^*)$  ($a^*=a[\kappa^*]$ etc.) so 
  \[
...
\frame{
  ...
  \begin{itemize}
  \item Localize in time: Local AWI (Yong '23.) arriving
    waves (usually) separated in time - localize via Gabor
    transform
  \item Microlocalize in space-time: dense source/receiver arrays,
    spatially extended virtual sources (``acoustic antennae''),
    penalize extension in space (Huang '19, S. '23) -
    asymptotic to non-standard traveltime misfit ($\sim$ lens rigidity), also in presence
    of multiple travel times
  \end{itemize}
  
  Completely untouched but extremely important: elasticity, EM,
  reflection (non-smooth coefficients)
}
...
\frame{
  ...
  \item Jianlian Qian, Shingyu Leung
  \end{itemize}
}
\end{document}
