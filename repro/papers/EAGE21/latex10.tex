...
\begin{frame}
Asymptotic inverse of modeling op:
\[
F^{\dagger} = \Lambda_s F^T \Lambda_r
\]
$\Lambda_{s,r}$ = pressure-to-velocity (``Dirichlet-to-Neumann'') operator on $z=z_s,z=z_r$

$F^{\dagger}F \approx$ {\em aperture-limited asymptotic identity}: preserves HF oscillatory pulses $\sim$ rays that travel from $z_s$ to $z_r$ = ``downgoing''
