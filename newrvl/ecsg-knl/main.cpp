...
int main(int argc, char* argv[])
{
...
#ifdef _OPENMP
    std::cout << "Number of OMP threads in use: " << omp_get_max_threads() << std::endl;
#endif
    ...
    //free surface (top_bc=1); Dirichlet surface (top_bc=0)
    int top_bc; par.get("top_bc", top_bc, 0);
    
    //read in source type
    int src_type; par.get("src_type", src_type, 1);
    
    //read in receiver data
    bool check_rec; par.get("check_rec", check_rec, false);
    ...
    if(check_rec) InitRecTrace(num_grids,grids,n_rec,rec_loc,rec_data);
    ...
    //+-+-+-+-+-+-+-+-+  START SIMULATION
    for(int niter = 0, iframe = 0; niter <= round(movie_dt/dt*movie_nt); niter++)
    {
        ...
        if(check_rec)
            ...
            {
                rec_data[0][i].push_back(sim[rec_loc[i][0]][0].GetVal(rec_loc[i][1],rec_loc[i][2],rec_loc[i][3]));
                rec_data[1][i].push_back(sim[rec_loc[i][0]][1].GetVal(rec_loc[i][1],rec_loc[i][2],rec_loc[i][3]));
                rec_data[2][i].push_back(sim[rec_loc[i][0]][2].GetVal(rec_loc[i][1],rec_loc[i][2],rec_loc[i][3]));
            }
        ...
        //=== add source
        if(niter < src_len)
            if(src_type == 0) //type 0: explosive source
            {
            ...
            else if(src_type == 1) //type 1: body force added only to vx
                sim[src_grid][0].SetVal(src_loc[0],src_loc[1],src_loc[2]) += 1000*src_trace[niter];
            else if(src_type == 2) //type 2: the source is a w(t)g(x), g(x) being gaussian wavelet
            {
                Field &vx = sim[src_grid][0];
                real peak=10; //control height of gaussian
                real width=10; //control width of gaussian
                real step1_pow = grids[src_grid].step[0]*grids[src_grid].step[0];
                real step2_pow = grids[src_grid].step[1]*grids[src_grid].step[1];
                real step3_pow = grids[src_grid].step[2]*grids[src_grid].step[2];
#ifdef _OPENMP
#pragma omp parallel for
#endif
                for(int iz = vx.GetStart(2)+2; iz < vx.GetEnd(2)-2; iz++)
                    for(int iy = vx.GetStart(1)+2; iy < vx.GetEnd(1)-2; iy++)
                        for(int ix = vx.GetStart(0)+2; ix < vx.GetEnd(0)-2; ix++)
                            vx.SetVal(ix,iy,iz) += src_trace[niter]*
                            peak*exp(-((ix-src_loc[0])*(ix-src_loc[0])*step1_pow
                                       +(iy-src_loc[1])*(iy-src_loc[1])*step2_pow
                                       +(iz-src_loc[2])*(iz-src_loc[2])*step3_pow)/width);
            }
        ...
        //===test SBP property (only for uniform grid)
        //        if(check_energy)
        //            TestSBP(sim);
    ...
    if(check_rec) DelRecTrace(n_rec,rec_loc,rec_data);
