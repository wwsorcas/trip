...
#ifdef _OPENMP
#pragma omp parallel for default(shared) schedule(static, OMP_CHUNK_SIZE)
#endif
    for(int i2 = start[2]; i2 <= end[2]; i2++)
        for(int i1 = start[1]; i1 <= end[1]; i1++)
#pragma ivdep
...
#ifdef _OPENMP
#pragma omp parallel for default(shared) firstprivate(rhs) schedule(static, OMP_CHUNK_SIZE)
#endif
    for(int i2 = start[2]; i2 <= end[2]; i2++)
        for(int i1 = start[1]; i1 <= end[1]; i1++)
#pragma ivdep
