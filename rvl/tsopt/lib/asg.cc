...
namespace RVL {
  ...
  void ASGapplyFO::operator()(TSDC & y) const {
	...
	throw e;
      ...
      */
      ...
      int ibulk = aux.gdom.get_ind().at("bulk");
      int ibuoy = aux.gdom.get_ind().at("buoy");
      int ip0 = aux.gdom.get_ind().at("p0");
      int ip1 = aux.gdom.get_ind().at("p1");
      int iv0 = aux.gdom.get_ind().at("v0");
      int iv1 = aux.gdom.get_ind().at("v1");
      GridDC & bulkdc = dynamic_cast<GridDC &>(ctrl[ibulk]);
      GridDC & buoydc = dynamic_cast<GridDC &>(ctrl[ibuoy]);	
      GridDC & p0dc = dynamic_cast<GridDC &>(state[ip0]);
      GridDC & p1dc = dynamic_cast<GridDC &>(state[ip1]);      
      GridDC & v0dc = dynamic_cast<GridDC &>(state[iv0]);
      GridDC & v1dc = dynamic_cast<GridDC &>(state[iv1]);

      IPNT gsc0; IPNT gec0;
      IPNT gsc1; IPNT gec1;
      
      if (integerstep) {
	aux.gdom.get_gc(gsc0,gec0,ip0);
	  ///////////////
	for (int i1=gsc1[1]+aux.nls[1];
	     i1<=gec1[1]-aux.nrs[1];i1++) {
	  for (int i0=gsc0[0]+aux.nls[0];
	       i0<=gec0[0]-aux.nrs[0];i0++) {
	...
	aux.gdom.get_gc(gsc0,gec0,iv0);
	aux.gdom.get_gc(gsc1,gec1,iv1);
  ...
  void ASGaux::initialize(GridDomain const & gdom,
	...
	lbc[i]=1; rbc[i]=1;
      ...
      IPNT gsc; IPNT gec;
      gdom.get_gc(gsc,gec,gdom.get_ind()["p0"]);
      vdiv_alloc
	= (float *)usermalloc_((gec[0]-gsc[0]+1)*sizeof(float));
      vdiv = &(vdiv_alloc[-gsc[0]]);
	...
	vstr<<"v"<<i;
	gdom.get_gc(gsc,gec,gdom.get_ind()[vstr.str()]);
	pgrad_alloc[i] = (float *)usermalloc_((gec[0]-gsc[0]+1)*sizeof(float));
	pgrad[i] = &(pgrad_alloc[i][-gsc[0]]);
      }
	...
	ep_alloc[idim]=NULL;
	epp_alloc[idim]=NULL;
	gdom.get_ga(gsa,gea,gdom.get_ind()[ips.str()]);
	asg_pmlaxis(gea[idim]-gsa[idim]+1,
		    nls[idim], nrs[idim],
		    amp, dt, 0,
		    &((ep_alloc)[idim]), &((epp_alloc)[idim]));
	ep[idim]=ep_alloc[idim] - gsa[idim];
	epp[idim]=epp_alloc[idim] - gsa[idim];

	std::stringstream ivs;
	ivs<<"v"<<idim;
	ev_alloc[idim]=NULL;
	evp_alloc[idim]=NULL;
	gdom.get_ga(gsa,gea,gdom.get_ind()[ivs.str()]);	
	asg_pmlaxis(gea[idim]-gsa[idim]+1,
		    nls[idim], nrs[idim],
		    amp, dt, 1,
		    &((ev_alloc)[idim]), &((evp_alloc)[idim]));
	ev[idim]=ev_alloc[idim] - gsa[idim];
	evp[idim]=evp_alloc[idim] - gsa[idim];
      }
