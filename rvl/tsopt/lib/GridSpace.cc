...
namespace RVL {
  ...
  */
  ...
  GridDomain::GridDomain(std::vector<Grid *> ctrlgrids,
    ...
    : TSSpace<float>(2) {
    dxs.resize((*(ctrlgrids[0])).getDimension());
    ind = _ind;
  ...
  void GridDomain::get_ga(IPNT gsa, IPNT gea, int i) const {
    try {
      TSSpace<float> const & ssp = dynamic_cast<TSSpace<float> const &>((*this)[1]);
      GridSpace const & gsp = dynamic_cast<GridSpace const &>(ssp[i]);
      gsp.getGrid().getAxisLimits(gsa,gea);
    }
    catch (bad_cast) {
      RVLException e;
      e<<"Error: GridDomain::get_ga\n";
      e<<"  structural error!\n";
      throw e;
    }
  }
  
  void GridDomain::get_gc(IPNT gsc, IPNT gec, int i) const {
    try {
      TSSpace<float> const & ssp = dynamic_cast<TSSpace<float> const &>((*this)[1]);
      GridSpace const & gsp = dynamic_cast<GridSpace const &>(ssp[i]);
      gsp.getGrid().getSubAxisLimits(gsc,gec);
    }
    catch (bad_cast) {
      RVLException e;
      e<<"Error: GridDomain::get_gc\n";
      e<<"  structural error!\n";
      throw e;
    }
  }
