...
namespace RVLUmin {
  ...
  */
  ...
  class ReduceOp: public Operator<Scalar>, public LSPolicy {
  ...
  private:
	...
	if (!applied) {
	  ...
	  cxx[1].zero();
	  Algorithm * solver
	    = LSPolicy::build(cxx[1],lop,d,rnorm,nrnorm,str);
	  solver->run();          
  ...
  protected:
	...
	e<<"\ncalled from ReduceOp::applyDeriv\n";
	throw e;
      }
    }
	...
	e<<"\ncalled from ReduceOp::applyAdjDeriv\n";
	throw e;
      }
    }

    Operator<Scalar> * clone() const {
      return new ReduceOp<Scalar,LSPolicy,LSPolicyData>(*this);
    }
        
  public:
      
    ReduceOp(LinOpValOp<Scalar> const & _op,
	...
	if (s.verbose) {
	  str<<"\n";
	  str<<"==============================================\n";
	  str<<"ReduceOp constructor - ls policy data = \n";
	  s.write(str);
	}
      }
      catch (RVLException & e) {
	e<<"\ncalled from ReduceOp::Constructor\n";
	throw e;
      }
    }
      
    ReduceOp(ReduceOp<Scalar,LSPolicy,LSPolicyData> const & f) 
      : LSPolicy(f), op(f.op), d(f.d), xx(op.getProductDomain()), cxx(xx),
