...
namespace RVLUmin {
  ...
  */
  ...
  public:
    ...
    */
	...
	while (active && dcount<5) {
      ...
      if (M) {
	solver = pol.build(p,opeval.getDeriv(),*M,
			   opeval.getValue(),rnorm,nrnorm,str);
      }
      else {
	solver = pol.build(p,opeval.getDeriv(),
			   opeval.getValue(),rnorm,nrnorm,str);
      }	
      agnrm=nrnorm;
    ...
    */
    TRGNStep(Policy const & _pol,
	     ...
	     shared_ptr<LinearOp<Scalar> const> _M,
	     atype _eta1,
	     atype _eta2,
	     atype _gamma1,
	     atype _gamma2,
	...
	throw e;
      ...
      // mod 2019.02.21 WWS: precond option
  ...
  private:
    ...
    shared_ptr<RVL::LinearOp<Scalar> const> M;
    atype & predred;
    atype & actred;
    atype & jval;
    atype & agnrm;
  ...
  */
  ...
  public:
	...
	TRGNStep<Scalar, Policy> step(*this,opeval,M,
				      eta1,eta2,gamma1,gamma2,minstep,
				      nostep,predred,actred,
				      jval,agnrm,rgnrm,str);
    ...
    */
    ...
    /** second form - preferred. Provides for preconditioner. Used if shared
	pointer is not null. 

	Parameters:
	@param op - operator figuring in least-squares problem
	@param M - shared pointer to linear op (preconditioner)
	@param x - solution Vector - initial guess on call, estimated solution on return
	@param _maxcount - max permitted TR steps (calls to TRGNStep)
	@param _jtol - stopping tolerance for least squares residual
	@param _agtol - absolute stopping tolerance for gradient norm
    ...
    */
    ...
    TRGNAlg(Operator<Scalar> const & op,
	    std::shared_ptr<RVL::LinearOp<Scalar> const> _M,
	    Vector<Scalar> & x,
	    int _maxcount,
	    atype _jtol,
	    atype _agtol,
  ...
  private:
    OperatorEvaluation<Scalar> opeval;
    shared_ptr<LinearOp<Scalar> const> M;
